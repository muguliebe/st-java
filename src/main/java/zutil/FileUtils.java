...
public class FileUtils {
	...
	public static List<String> readLargeFileLines(String name) {
		try {
			BufferedReader reader = Files.newBufferedReader(
					FileSystems.getDefault().getPath("", name), Charset.defaultCharset());
	...
	public static void writeFileBytes(String filename, String content) {
		try {
			Files.write(FileSystems.getDefault().getPath(".", filename), content.getBytes(),
	...
	public static void writeFileBytesBuffered(String filename, String content) {
		try {
			BufferedWriter writer = Files.newBufferedWriter(
					FileSystems.getDefault().getPath(".", filename), Charset.forName("UTF-8"),
					StandardOpenOption.CREATE);
	...
	public static List<Path> getDirectories(final String dir) throws IOException {
		...
		try (DirectoryStream<Path> stream = Files
				.newDirectoryStream(FileSystems.getDefault().getPath(dir))) {
	...
	public static List<Path> getEntries(final String dir,
			final DirectoryStream.Filter<? super Path> filter) throws IOException {
		final List<Path> entries = new ArrayList<>();
		try (DirectoryStream<Path> stream = Files
				.newDirectoryStream(FileSystems.getDefault().getPath(dir), filter)) {
			for (final Iterator<Path> it = stream.iterator(); it.hasNext();) {
				entries.add(it.next());
			}
		}
	...
	public static List<Path> getEntries(final String dir, final String regularExpression)
			throws IOException {
		final List<Path> entries = new ArrayList<>();
		try (DirectoryStream<Path> stream = Files
				.newDirectoryStream(FileSystems.getDefault().getPath(dir), regularExpression)) {
			for (final Iterator<Path> it = stream.iterator(); it.hasNext();) {
				entries.add(it.next());
			}
		}
